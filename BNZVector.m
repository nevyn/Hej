...
+(BNZVector*)vectorX:(double)x y:(double)y z:(double)z;
{
	double *v = malloc(sizeof(double)*3);
	v[0] = x;
	v[1] = y;
	v[2] = z;
	return [BNZVector vectorWithSize:3 CArray:v isResponsible:BNZVectorMemoryFreeResponsible];
}
+(BNZVector*)vectorFromPoint:(NSPoint)point;
{
...
-(NSPoint)asPoint;
{
    return NSMakePoint(values[0], values[1]);
}
-(CGPoint)asCGPoint;
{
